...
## v21 startup fixes
- PlayCanvas is pinned to a known current engine build.
- `pc.Application` receives the actual canvas element.
- The menu is hidden only after the engine and scene initialize successfully.
- Startup errors are shown instead of leaving a black screen.
- Fixed the first-shot weapon animation DOM/engine object mismatch.
