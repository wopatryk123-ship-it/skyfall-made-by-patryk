let token='',profile=null,ws=null,players={},remote={},room='PUBLIC',mapName='neon',mode='ffa',myTeam=null; window.pickupState={};
const W={
...
function fire(){if(!alive||reloading||cool>0||ammo<=0||!ws||ws.readyState!==1)return;ensureAudio();ammo--;cool=W[weapon].rate;recoil=W[weapon].recoil;weaponAnim=1;playShotSound();if(muzzle){muzzle.enabled=true;muzzle.setLocalScale(1.8,1.8,1.8);clearTimeout(flashTimer);flashTimer=setTimeout(()=>{if(muzzle){muzzle.enabled=false;muzzle.setLocalScale(1,1,1)}},55)}arms.classList.remove('fireanim');void arms.offsetWidth;arms.classList.add('fireanim');setTimeout(()=>arms&&arms.classList.remove('fireanim'),130);ws.send(JSON.stringify({type:'fire',weapon}));updateAmmo();if(ammo===0)reload()}
...
function startGame(){if(!token||!profile){$('msg').textContent='Najpierw zaloguj się.';return}if(typeof pc==='undefined'){$('msg').textContent='Nie załadował się silnik gry. Sprawdź internet i odśwież stronę.';return}mapName=$('mapSel').value;mode=$('modeSel').value;room=($('room').value.trim().toUpperCase()||'PUBLIC');$('menu').style.display='none';app=new pc.Application('pc',{graphicsDeviceOptions:{alpha:false,antialias:true}});app.start();app.setCanvasFillMode(pc.FILLMODE_FILL_WINDOW);app.setCanvasResolution(pc.RESOLUTION_AUTO);buildMap();connect();setupLook();setupTouch();setupKeyboard();app.on('update',tick);updateAmmo();updateGrenades()}
