const SKYFALL_VERSION='v22'; let token='',profile=null,ws=null,players={},remote={},room='PUBLIC',mapName='neon',mode='ffa',myTeam=null; window.pickupState={};
const W={
...
function setupKeyboard(){const keys={};const syncKeys=()=>{const kx=(keys.KeyD?1:0)-(keys.KeyA?1:0),kz=(keys.KeyS?1:0)-(keys.KeyW?1:0);if(kx||kz){move.x=kx;move.z=kz}else if(!touchMoveActive){move.x=0;move.z=0}};addEventListener('keydown',e=>{keys[e.code]=true;if(e.code==='ShiftLeft'||e.code==='ShiftRight')sprint=true;if(e.code==='Space'){e.preventDefault();jump()}syncKeys()});addEventListener('keyup',e=>{keys[e.code]=false;if(e.code==='ShiftLeft'||e.code==='ShiftRight')sprint=false;syncKeys()});app.on('update',syncKeys)}
...
function setupTouch(){const joy=$('joy'),knob=$('knob'),mobile=$('.mobile');let id=null;touchMoveActive=false;const stopJoy=()=>{id=null;touchMoveActive=false;move={x:0,z:0};sprint=false;knob.style.transform=''};joy.addEventListener('pointerdown',e=>{e.preventDefault();id=e.pointerId;touchMoveActive=true;joy.setPointerCapture(id);moveJoy(e)});joy.addEventListener('pointermove',e=>{if(e.pointerId===id)moveJoy(e)});joy.addEventListener('pointerup',stopJoy);joy.addEventListener('pointercancel',stopJoy);joy.addEventListener('lostpointercapture',stopJoy);function moveJoy(e){const r=joy.getBoundingClientRect(),x=e.clientX-r.left-r.width/2,y=e.clientY-r.top-r.height/2,d=Math.min(42,Math.hypot(x,y)),a=Math.atan2(y,x),nx=Math.cos(a)*d/42,ny=Math.sin(a)*d/42;knob.style.transform=`translate(${nx*42}px,${ny*42}px)`;move={x:nx,z:ny};sprint=d>30}const fireBtn=$('fire');fireBtn.addEventListener('pointerdown',e=>{e.preventDefault();fireHeld=true;fire()});['pointerup','pointercancel','pointerleave'].forEach(ev=>fireBtn.addEventListener(ev,()=>fireHeld=false));$('reload').addEventListener('pointerdown',e=>{e.preventDefault();reload()});$('jump').addEventListener('pointerdown',e=>{e.preventDefault();jump()});$('exit').addEventListener('pointerdown',e=>{e.preventDefault();try{ws?.close()}catch{};location.reload()});$('grenade').addEventListener('pointerdown',e=>{e.preventDefault();throwGrenade()});if(mobile)mobile.addEventListener('touchmove',e=>e.preventDefault(),{passive:false})}
