...
const PORT=process.env.PORT||3000,SECRET=process.env.JWT_SECRET||'CHANGE_ME_NOW',DB=path.join(__dirname,'data.json');
let db=fs.existsSync(DB)?JSON.parse(fs.readFileSync(DB,'utf8')):{users:{}};
app.disable('x-powered-by');
app.use(express.json({limit:'32kb'}));
app.get('/health',(req,res)=>res.json({ok:true,game:'SKYFALL',version:'v20'}));
app.get('/api/ping',(req,res)=>res.json({ok:true,version:'v20'}));
...
server.listen(PORT,'0.0.0.0',()=>console.log(`SKYFALL v20 server on http://localhost:${PORT}`));
